...
Until the current gate passes, do not mutate the project. Native file reads, native searches, directory listings, asking the user a question, planning, loading a skill, dispatching a subagent, reasoning, explaining, and gathering information are allowed. Network tools such as web fetch and web search are unavailable before pass unless the host operator has opted in. Shell commands are unavailable before pass, including commands that appear read-only, except for an exact provider-specific control command or Codex inspection command shown above. File writes, edits, deletes, and notebook edits are also unavailable.
