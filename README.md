...
Nothing trims git's output beyond its terminating newline. A directory whose name ends in a space is a different directory, and trimming it examined the neighbour and reported *its* change set as this one's — not a failure but a wrong answer, which is worse.
