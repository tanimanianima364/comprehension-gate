...
State is stored outside the project and keyed by a SHA-256 digest of provider plus `session_id`. Codex `turn_id` and Cursor `generation_id` are also checked when available; a pending state seeded by an event without a turn id adopts the first turn id it sees so that turn can still pass. Claude Code supplies no turn id, so each reset records the identity (byte offset plus `uuid`) of the latest human prompt record in the tail of `transcript_path`, and every `PreToolUse` compares it with the current latest record; if the prompt hook was skipped or timed out, a newer record resets the gate to pending instead of inheriting the previous pass, even when the prompt text is identical. If the transcript cannot be judged while the gate is satisfied, the gate returns to pending and the re-pass sticks until a record is judgeable again. If the first observed lifecycle event is `PreToolUse`, a missing state is initialized to pending before policy evaluation; invalid or unreadable state remains fail-closed. Writes use a temporary file and rename so parallel hook processes never observe partially written JSON, and each armed control is recorded in its own per-tool-use file (per-action when the host omits `tool_use_id`) bound to the current request sequence, so concurrent control reads cannot overwrite each other; the first successful completion wins and later ones are no-ops. Unrecognized or missing `hook_event_name` values, unparseable hook input, and `SessionStart` failures exit non-zero rather than emitting an allow or a `PreToolUse`-shaped payload. Set `COMPREHENSION_GATE_STATE_DIR` to override the state directory for tests or managed deployments.
