...
The plugin is developed and tested on Linux, including WSL, and supports shell commands only there. Anywhere else the gate refuses every shell tool: the classifier's rules read a command as a POSIX shell command, and applying them to another shell would let a PowerShell cmdlet such as `Remove-Item` through as inspection, since it matches no name in the table. Rather than guess at a shell it does not model, the gate declines.

The refusal only restricts, so the gate still works elsewhere with less available to it: native reads and searches are untouched, so Claude Code, Cursor, and Kiro still inspect and still pass through a native read. Codex, whose controls and inspection bridge are shell commands, is Linux-only in practice.
