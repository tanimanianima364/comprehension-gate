...
Comprehension Gate layers a deterministic insight gate on top of an agent's learning behavior. It does not fork or replace the official `learning-output-style` plugin.

The hook never refuses a tool. It records the last accounted-for state of the repository, and when the agent tries to end a turn in which the repository changed without accounting for it, it holds the turn open on hosts that allow it. At MEDIUM the agent owes the user a short insight about the convention or principle the change touched; at HIGH and CRITICAL it also owes one transfer question about where that principle reaches beyond the change. The agent still decides whether a change is LOW, MEDIUM, HIGH, or CRITICAL and judges the answer; the hook only owns turn control.

## Supported adapters

| Agent | Hook configuration | Holds the turn | Verified against a running host |
| --- | --- | --- | --- |
| Claude Code | `hooks/hooks.json` | yes | yes, 2.1.258: `decision: "block"` holds once, and `systemMessage` is shown to the user on both the blocking Stop and the second (allowed) Stop; re-verified on 2.1.259 for the insight gate, where a MEDIUM change passes inside the turn with no hold and a HIGH change holds once on the transfer question and passes only after the answer |
| Codex | the same `hooks/hooks.json` | yes (hold observed on 0.153.1) | yes, CLI 0.151.0: Codex discovers `hooks/hooks.json` from the installed plugin, supplies `CLAUDE_PLUGIN_ROOT`, runs `SessionStart`/`PreToolUse`/`PostToolUse`, and completes the pinned control command; on 0.151.0 a hold was never observed because the model completed a control before the turn ended in every run, but on 0.153.1 the hold was observed live: the `Stop` returned `decision: "block"`, Codex honored it with no intervening tool call, and the second Stop was allowed |
| Cursor | Claude compatibility or `adapters/cursor/hooks.json` | follow-up message (cannot hold) | hooks yes, `cursor-agent` 2026.09.02: every hook fires and the stop follow-up was confirmed against the state file. Controls failed there, because Cursor's `Read` and `Grep` `postToolUse` payloads carry no file content for the marker to be found in; they are now confirmed from the read's path and byte count instead, which is covered by a test built from the captured payload but has not itself been re-run live |
| Kiro CLI 2.x | `adapters/kiro-2x/hooks.json`, merged into the agent config | warning only | yes, 2.16.2: verified end to end with the real hook command: baseline at `agentSpawn`, `stop` exits 1 with the reason, an outstanding notice on the next `userPromptSubmit`, and the baseline not retaken until a control completes; re-verified for the insight gate, where a MEDIUM change leaves no outstanding state at all and `stop` exits 0 |
| Kiro CLI 3.x | `adapters/kiro/hooks.json` | warning only | no |

The last column is the honest one. Claude Code, Codex, Cursor, and Kiro CLI 2.x have all been exercised against a running host. Cursor was the one that did not work end to end: its hooks fire and its `preToolUse` arms a control correctly, but the matching `postToolUse` carries only `{"file_path": …, "content_length": …}` for a `Read` and `{"pattern": …, "success": true}` for a `Grep`, so the marker text was never present and `controlTransitionSucceeded` could never confirm — leaving the agent to report a pass that had not happened while the state stayed outstanding. Cursor now confirms from the path and the byte count, which a test models from that captured payload; that path has not been re-run against a live Cursor. Separately, a `cursor-agent --resume` turn fires no hooks at all, not even `sessionStart`. Kiro CLI 3.x has not been exercised live; that adapter was written from vendor documentation and is covered by tests that model the documented contract, not by a live run. It is also still early access at the time of writing, reached with `kiro-cli --v3`, so 2.x is what most installations have.
...
Stop -> compare the repository (every worktree, ignored files excluded) with the baseline
  no difference -> allow, dropping any outstanding record
  otherwise -> record the change; Claude Code holds once, Cursor injects a follow-up, Kiro warns
```

A snapshot covers every worktree of the git repository that contains the hook's working directory (identified by `git rev-parse --git-common-dir` plus `git worktree list --porcelain`, so a worktree created anywhere is still inside the project). For each worktree it records the HEAD commit id and, for every path `git status --porcelain=v1 -z --untracked-files=all` reports, a content hash; a commit shows up as a HEAD change, so a turn that edits and commits still counts. Ignored files are never listed, so anything under a gitignored directory, in `/tmp`, or in the host's scratchpad is free. `git status` never descends into a nested repository or a submodule, reporting it as a single entry, so such an entry is hashed from the path, size and modification time of every file beneath it; that walk stops after 2000 files, and changes beyond the first 2000 files of a nested repository are not seen. The baseline is the snapshot of the last accounted-for state: it is retaken whenever a control action (pass or LOW bypass) completes (the two are one record: when the snapshot cannot be taken at the control in a tracked project, the control is not recorded and the agent is told to perform it again), and at `SessionStart` and `UserPromptSubmit` only when nothing is outstanding, so edits the user made between turns are theirs and are never put to them as a question, while a change the gate is already holding is never adopted as the new baseline by a `SessionStart` over it. That protection reaches as far as the session id does: state is keyed by a digest of provider and session id, so a host that mints a new id for a start (a `/clear`, for example) lands on a fresh state file, finds no record, and baselines the current tree. A directory that is not a git repository, or a snapshot that fails to capture (a corrupt index, for example), leaves the gate inactive for that session; it never blocks a prompt.

At `Stop` the hook compares the current snapshot with the baseline. With no difference it allows, and drops any outstanding record, since the tree is back at the accounted-for state whoever undid the change. Otherwise it records the changed paths and acts per host. A pass covers the tree as it stood when the control completed, which is the baseline it retook: writes made before the pass in the same turn are covered, and a write made after it, to the same file or another, is a new change that holds like any other. On Claude Code, the first Stop of a turn (`stop_hook_active` false) returns `decision: "block"` with the changed paths, so the agent accounts for the change instead of finishing; whatever it writes ends the turn, and the second Stop (`stop_hook_active` true) allows and returns a `systemMessage` telling the user an unaccounted change remains. The Stop hook runs either way; the blocking path is reached only when the level was not satisfied inside the turn, which at MEDIUM it usually is: the insight needs no answer, so the control completes in the same turn and nothing is held. This holds the turn once, never twice. Cursor cannot hold the turn; when `status` is `"completed"` and `loop_count` is `0` it returns the hold reason as a `followup_message` (`loop_limit: 1` in the adapter), and returns `{}` otherwise, including after an aborted turn. Cursor's prompt hook carries no context field, so on Cursor the outstanding notice reaches the agent only through the stop follow-up message. Kiro CLI, on both 2.x and 3.x, can only warn: the hook exits non-zero with the hold reason on stderr, which the host shows as a warning, and the next `UserPromptSubmit` injects the outstanding notice into context. `SubagentStop` is not hooked; a subagent's writes are part of the diff the main `Stop` sees.
