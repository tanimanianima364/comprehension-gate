...
Stdout stays bytes until the last moment. A file name is bytes, and decoding it as UTF-8 early replaces every invalid byte with the same character, so two differently named files became one path and one of them disappeared; a name that is not valid UTF-8 is escaped byte by byte instead, which keeps it distinct.
